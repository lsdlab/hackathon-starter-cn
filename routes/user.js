...
router.post('/account/password', passportConfig.isAuthenticated, function(req, res, next) {
  User.findUserByID(req.user.id).then(function(user) {
    if (!_.isEmpty(user)) {
      var newPassword = req.body['old-password']
      User.comparePassword(newPassword, user.password).then(function(isMatch) {
        if (isMatch === true) {
          req.assert('password', 'Password must be at least 8 characters long').len(8)
          req.assert('confirmPassword', 'Passwords do not match').equals(req.body.password)
          ...
          User.generateHashedPassword(newPassword).then(function(hashedPassowrd) {
            User.updatePassword(hashedPassowrd, user.id).then(function() {
